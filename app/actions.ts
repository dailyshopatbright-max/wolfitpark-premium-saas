...
async function save(type: string, data: Record<string, string>) {
  const res = await fetch("/api/submissions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, data }) })
...
export async function submitContact(_: FormState, formData: FormData): Promise<FormState> {
  ...
  try { await save("contact", parsed.data); return {status:"success",message:"Message received. Our team will respond within one business day."} } catch { return {status:"error",message:"We could not send your message. Please email support@wolfitpark.online."} }
}
export async function submitConsultation(_: FormState, formData: FormData): Promise<FormState> {
  ...
  try { await save("consultation", parsed.data); return {status:"success",message:"Your consultation request is in. We will follow up shortly."} } catch { return {status:"error",message:"We could not send your request. Please email support@wolfitpark.online."} }
}
export async function submitCareer(_: FormState, formData: FormData): Promise<FormState> {
  ...
  try { await save("career", parsed.data); return {status:"success",message:"Application received. We will review it carefully and contact selected candidates."} } catch { return {status:"error",message:"We could not send your application. Please email support@wolfitpark.online."} }
}
export async function submitNewsletter(_: FormState, formData: FormData): Promise<FormState> {
  ...
  try { await save("newsletter", parsed.data); return {status:"success",message:"You are on the list. Watch your inbox."} } catch { return {status:"error",message:"Subscription failed. Please try again later."} }
}
